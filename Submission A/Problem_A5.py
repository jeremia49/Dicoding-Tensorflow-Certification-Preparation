...
class StopCallback(tf.keras.callbacks.Callback):
    def on_epoch_end(self, epoch, logs=None):
        # keys = list(logs.keys())
        if(epoch > 15):
          if(logs['mean_absolute_error'] < 0.15):
...
def solution_A5():
		...
		model.compile(loss=tf.keras.losses.Huber(), optimizer=tf.keras.optimizers.SGD(momentum=0.9), metrics=[tf.keras.metrics.MeanAbsoluteError()])

		model.fit(train_set, epochs=100, callbacks=[StopCallback()])
